...
func (s *server) GetStream(req *pb.HelloRequest, res pb.Greeter_GetStreamServer) error {
	if req.Data == "hello" {
		for i := 0; i < 6; i++ {
...
func (s *server) SetStream(req pb.Greeter_SetStreamServer) error {
...
func (s *server) AllStream(req pb.Greeter_AllStreamServer) error {
	w := sync.WaitGroup{}
	w.Add(1)
	go func() {
		for {
